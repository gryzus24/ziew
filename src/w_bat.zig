...
const Battery = struct {
    ...
    pub fn checkPairs(self: @This(), ac: color.Active, base: [*]const u8) color.Hex {
        return switch (typ.BatOpt.ColorSupported = @enumFromInt(ac.opt)) {
    ...
    for (Parser.checks[state..]) |check| {
        ...
        if (@ctz(m) == key_len_complement) {
            ...
            const value: usize = switch (key) {
                .status => switch (vstr[0] & (0xff - 0x20)) {
                    'D' => @intFromEnum(Battery.State.discharging),
                    'C' => @intFromEnum(Battery.State.charging),
                    'F' => @intFromEnum(Battery.State.full),
                    'N' => @intFromEnum(Battery.State.notcharging),
                    else => @intFromEnum(Battery.State.unknown),
                },
...
pub noinline fn widget(writer: *uio.Writer, w: *const typ.Widget, base: [*]const u8) void {
    ...
    for (wd.format.parts.get(base)) |*part| {
        ...
        if (opt == .state or opt == .arg) {
            ...
            dst[0..SZ].* = switch (opt) {
                .state => Battery.State.names[bat.fields[Battery.state]],
                ...
                .arg => wd.path[wd.ps_off..].ptr[0..SZ].*,
                .@"%fulldesign", .@"%fullnow" => unreachable,
            };
            writer.end += SZ;
        } else {
